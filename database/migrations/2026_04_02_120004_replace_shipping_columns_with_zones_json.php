...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasColumn('products', 'shipping_zones')) {
            Schema::table('products', function (Blueprint $table) {
                $table->json('shipping_zones')->nullable()->after('free_shipping');
            });
        }

        // Migrate existing data (if columns exist from older schema)
        if (Schema::hasColumn('products', 'shipping_inside_dhaka')) {
        ...
        // Drop shipping_charge if it exists (from add_shipping_to_products migration)
